...
@dataclass
class _NativeCompressOutputCollector:
...
class CompressorAscendBackendMixin(CompressorBackendMixin):
    ...
    def _native_chunked_followup(
        ...
        forward_batch: ForwardBatch,
        compress_output_collector: _NativeCompressOutputCollector,
        device: torch.device,
    ) -> None:
        ...
        if remainder > 0:
            ...
            compress_output_collector.kv_state_to_be_cached.append(kv[cutoff:chunk_len])
            compress_output_collector.score_state_to_be_cached.append(
                score[cutoff:chunk_len] + compressor.ape[gpos % ratio]
            )
            compress_output_collector.state_loc_list.append(
                state_row[gpos].to(torch.int64)
            )
        if overlap and should_compress:
            gpos = torch.arange(cutoff - ratio, cutoff, device=device) + prefix_len
            compress_output_collector.kv_state_to_be_cached.append(
        ...
        compress_output_collector.kv_out_list.append(kv_compressed)
        compress_output_collector.kv_out_positions.append(pos_req[:cutoff:ratio])
        compress_output_collector.write_req_indices.append(
            torch.full((n_out,), idx, dtype=torch.int64, device=device)
        )
        compress_output_collector.write_pos_in_req.append(
    ...
    def _forward_compress_native(
        ...
        * Non-prefill (one token/req): append (kv, score) to the state ring; if it
        ...
        for idx, seqlen in enumerate(seq_lens_cpu):
            ...
            if is_prefill:
                ...
                if remainder > 0:
                    ...
                    compress_output_collector.kv_state_to_be_cached.append(kv_rem)
                    compress_output_collector.score_state_to_be_cached.append(
                        score_rem + compressor.ape[:remainder]
                    )
                    compress_output_collector.state_loc_list.append(
                        out_cache_loc[-remainder:]
                    )
                    kv = kv_cut
                    score = score_cut
                ...
                if should_compress:
                    ...
                    compress_output_collector.kv_out_list.append(kv_compressed)
                    compress_output_collector.kv_out_positions.append(pos_compressed)
                    compress_output_collector.write_req_indices.append(
                        torch.full(
                ...
                if should_compress:
                    ...
                    compress_output_collector.kv_out_list.append(kv_compressed)
                    compress_output_collector.kv_out_positions.append(pos_req)
        ...
        if compress_output_collector.kv_state_to_be_cached:
            kv_state_cat = torch.cat(
                compress_output_collector.kv_state_to_be_cached, dim=0
            ).unsqueeze(1)
            score_state_cat = torch.cat(
                compress_output_collector.score_state_to_be_cached, dim=0
            ).unsqueeze(1)
            state_loc_cat = torch.cat(compress_output_collector.state_loc_list, dim=0)
            token_to_kv_pool.set_state_buffer(
                compressor.layer_id,
                state_loc_cat,
                kv_state_cat,
        ...
        if compress_output_collector.kv_out_list:
            kv_out = torch.cat(compress_output_collector.kv_out_list, dim=0).to(dtype)
            pos_out = torch.cat(compress_output_collector.kv_out_positions, dim=0)
            ...
            req_indices_flat = torch.cat(
                compress_output_collector.write_req_indices, dim=0
            )
            pos_in_req_flat = torch.cat(
                compress_output_collector.write_pos_in_req, dim=0
            )
